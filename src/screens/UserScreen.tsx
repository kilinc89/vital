...
export const UserScreen = ({route, navigation}) => {
  ...
  useEffect(() => {
    const subscription = VitalCore.observeStatusChange(status => {
      ...
      VitalCore.currentUserId().then(userId => {
        setIsCurrentSDKUser(
          userId?.toLowerCase() == user.user_id.toLowerCase(),
        );
      });
    });
    ...
    return (
      <Box padding="4" borderColor="#333333" borderWidth="1">
        ...
        <HStack alignItems={'center'} style={{marginTop: 8}}>
          ...
          <Button
            onPress={() =>
              VitalHealth.syncData(permissionAsked)
                .then(x => console.log(2222, x))
                .catch(y => {
                  console.log(3333, y);
                })
            }>
  ...
  return (
    <VStack px={4} py={4} space={4}>
      ...
      {!isCurrentSDKUser && isSDKConfigured && (
        <Button onPress={() => VitalCore.cleanUp()}>Reset SDK</Button>
      )}
